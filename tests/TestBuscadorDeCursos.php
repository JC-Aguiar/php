...
class TestBuscadorDeCursos extends TestCase
{
    ...
    {
        $buscador = new BuscadorGeral($this->url);
        $crawler = new Crawler();
        $buscador->setHttpClient($this->httpClientMock);
        $buscador->setNavegador($crawler);
        $cursos = $buscador->buscarArray($this->url,"span[class=card-curso__nome]");
